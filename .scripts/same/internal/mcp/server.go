// Package mcp implements the MCP server for vault-search.
package mcp
...
type searchInput struct {
	...
	TopK  int    `json:"top_k" jsonschema:"description=Number of results (default 10, max 100)"`
}

type searchFilteredInput struct {
	Query      string `json:"query" jsonschema:"description=Natural language search query"`
	TopK       int    `json:"top_k" jsonschema:"description=Number of results (default 10, max 100)"`
	Domain     string `json:"domain" jsonschema:"description=Filter by domain"`
	Workstream string `json:"workstream" jsonschema:"description=Filter by workstream"`
	Tags       string `json:"tags" jsonschema:"description=Comma-separated tags to filter by"`
}

type getInput struct {
	Path string `json:"path" jsonschema:"description=Relative path from vault root"`
}

type similarInput struct {
	Path string `json:"path" jsonschema:"description=Relative path of the source note"`
	TopK int    `json:"top_k" jsonschema:"description=Number of similar notes (default 5, max 100)"`
}

type reindexInput struct {
	Force bool `json:"force" jsonschema:"description=Re-embed all files regardless of changes"`
}
