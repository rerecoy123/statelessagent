...
# Usage: curl -fsSL https://raw.githubusercontent.com/sgxdev/statelessagent/main/.scripts/same/install.sh | bash

set -euo pipefail

REPO="sgxdev/statelessagent"
INSTALL_DIR="${INSTALL_DIR:-.scripts/bin}"
...
if [ -z "$LATEST" ]; then
  ...
  if command -v go >/dev/null 2>&1; then
    cd .scripts/same 2>/dev/null || { echo "Run from vault root directory"; exit 1; }
    CGO_ENABLED=1 go build -ldflags "-s -w" -o "../bin/same" ./cmd/same
    echo "Built .scripts/bin/same from source"
    exit 0
  fi
