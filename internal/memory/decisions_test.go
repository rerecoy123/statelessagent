...
func TestDecisionExtraction(t *testing.T) {
	// Try to load ground truth from eval data (if eval harness is present)
...
func testInlineDecisions(t *testing.T) {
	tests := []struct {
		...
		{
			// "Let's go with" matches the pattern, but requireRationale=true
			// and no rationale indicator is present, so it's filtered out.
			text:     "Let's go with the hook-based architecture for session handoffs because it's simpler.",
			wantAny:  true,
			wantText: "go with",
		},
		{
